...
const actions = {
  ...
  async loadImageCollectionAJAX (context, resource) {
    if (resource == null) {
      context.commit('CHANGE_RESOURCE_LOAD_STATE', 'LOADING_ERROR')
      console.error('Failed to retrieve the resource')
      return
    ...
    let resource_type = 'generic_work'
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4) {
        ...
        if (xhr.status === 200 || xhr.status === 201) {
          // Successful response
          context.commit('SET_RESOURCE', xhr.responseText)
          context.commit('CHANGE_RESOURCE_LOAD_STATE', 'LOADED')
        } else {
          // Handle errors here
          // console.error('Error:', xhr.status, xhr.statusText);
          context.commit('CHANGE_RESOURCE_LOAD_STATE', 'LOADING_ERROR')
          context.commit('ERROR_MESSAGE', xhr.statusText)
        }
      }
    };
